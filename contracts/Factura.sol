...
contract Factura is ERC721URIStorage{
    ...
    constructor(uint _id) ERC721('Titulos de Propiedad', 'TDP') {
        id = _id;
        owner = msg.sender;
    }

    function setPropietario(string memory _propietario) public {
        string memory finalSvg = string(abi.encodePacked(svgPartOne, id , _propietario, svgPartTwo));
        uint256 newRecordId = _tokenIds.current();
        uint256 length = StringUtils.strlen(_propietario);
